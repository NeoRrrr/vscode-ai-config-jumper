...
- Keep the `System` group visible when system configs are enabled. By default, show system configs inline inside the `System` group. When `aiConfigJumper.flattenSystemConfigs` is disabled, compact files under parent system directories, such as `~/.claude/settings.json` under `~/.claude`.
...
- Keep the tree fresh when relevant AI config files or directories are created or deleted. Watchers should filter events to known config names, configured custom paths, configured search roots, and system config candidates.
