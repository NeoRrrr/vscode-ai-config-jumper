...
- **系统区布局**：系统级配置默认按目录收纳在 `System` 分组里，也可以切换成分组内平铺显示。
- **右键快捷操作**：新建文件、打开、侧边打开、资源管理器定位、复制绝对路径、复制相对路径。
- **自动刷新**：相关 AI 配置文件或目录被创建、删除时，侧边栏会自动更新。
...
6. 相关配置被创建或删除时会自动刷新；需要时也可以点击刷新按钮重新扫描。
...
打开后，侧边栏会新增 `System` 分组。目录内的系统配置会默认收纳到对应目录下，例如 `~/.claude/settings.json` 会出现在 `~/.claude` 目录里。
...
`System` 分组内部默认直接平铺显示所有系统级配置：

```json
"aiConfigJumper.flattenSystemConfigs": true
```

如果你希望把目录内部的系统配置收纳到对应目录下，可以关闭它：

```json
"aiConfigJumper.flattenSystemConfigs": false
```
